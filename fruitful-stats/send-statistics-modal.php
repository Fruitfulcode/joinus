...
class ffc_joinus_stats_modal
{
	...
	public function __construct()
	{
		...
		add_action( 'wp_ajax_fruitful_joinus_submit_modal', array( $this, 'ffc_submit_modal' ) );

		// Add action on click close button modal notification
		add_action( 'wp_ajax_fruitful_joinus_dismiss_subscribe_notification', array( $this, 'ffc_dismiss_subscribe_notification' ) );
	}
	...
	public function ffc_submit_modal() {
		...
		$response = array(
			'status'            => 'failed',
			'title'             => __( 'Uh oh!', 'joinus' ),
			'error_message'     => __( 'Sorry, something went wrong, and we failed to receive the shared data from you.', 'joinus' ),
			'error_description' => __( 'No worries; go to the theme option to enter the required data manually and save changes.', 'joinus' ),
			'stat_msg'          => '',
			'subscr_msg'        => ''
		);
		...
		if ( ! empty( $request_data ) ) {
			...
			if ( $request_data['ffc_statistic'] === 1 || $request_data['ffc_subscribe'] === 1 ) {
				$response = array(
					'status'            => 'success',
					'title'             => __( 'Thank you!', 'joinus' ),
					'error_message'     => '',
					'error_description' => '',
					'stat_msg'          => __( 'Thank you for being supportive, we appreciate your understanding and assistance!', 'joinus' ),
					'subscr_msg'        => $request_data['ffc_subscribe'] === 1 ? __( "Don't forget to check your inbox for our latest letter - you’d like that!", 'joinus' ) : ''
				);
			} else {
				$response = array(
					'status'            => 'success',
					'title'             => __( 'What a pity!', 'joinus' ),
					'error_message'     => '',
					'error_description' => '',
					'stat_msg'          => __( 'We wish you could have shared your site statistic and joined our community.', 'joinus' ),
					'subscr_msg'        => __( 'But if you ever change your mind, you can always do that in the theme options.', 'joinus' )
				);
			}
		}
